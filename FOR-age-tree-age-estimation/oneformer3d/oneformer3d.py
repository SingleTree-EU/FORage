...
from torch_scatter import scatter_mean, scatter_add, segment_csr
...
import os
import numpy as np
from tools.base_modules import Seq, MLP, FastBatchNorm1d, MLP_2
...
class ForAINetV2OneFormer3D_XAwarequery(Base3DDetector):
    ...
    def finalize_semantic_labels(all_pre_sem: list[list[int]]) -> np.ndarray:
        """
        all_pre_sem  : List[ List[int] ]，len == N_total
                    每个点的投票结果（可为空 list）

        返回：
            np.ndarray (N_total,) – 多数票语义标签；若无投票则 -1
        """

        # ① 统计 list 长度，拼接成一条 1‑D 数组
        ...
        for idx, (s, e) in enumerate(zip(starts, offsets)):
            if s == e:             # 没有投票
                continue
            # bincount + argmax 比 Counter 快一个数量级
            slice_vals = flat[s:e]
...
@MODELS.register_module()
class ForestFormerDownstream(Base3DDetector):
    r"""ForSpecies dataset.
    ...
    def __init__(self,
        ...
        #self.Classifier = (
        #    Seq()  
        #    .append(MLP_2([num_channels, num_channels], bias=False))
        #    .append(torch.nn.Linear(num_channels, num_channels // 2))
        #    .append(torch.nn.LeakyReLU(negative_slope=0.1))
        #    .append(torch.nn.Linear(num_channels // 2, self.num_classes))
        #    .append(torch.nn.LogSoftmax(dim=-1))
        #)
        self.Classifier = torch.nn.Sequential(
            torch.nn.Linear(num_channels, 256),
    ...
    def loss22(self, batch_inputs_dict, batch_data_samples, **kwargs):
        """Calculate losses from a batch of inputs dict and data samples.
        ...
        encoder_prefixes = (
            ...
            'u.blocks.', 'u.conv.',
            'u.u.blocks.', 'u.u.conv.',
            'u.u.u.blocks.', 'u.u.u.conv.',
            'u.u.u.u.blocks.', 'u.u.u.u.conv.',
        )
...
'''
        #np.savetxt("points_xyz_input.txt", batch_inputs_dict["points"][0].cpu().numpy(), fmt="%.6f", delimiter=" ")

        coordinates, features, inverse_mapping, spatial_shape = self.collate(
            batch_inputs_dict['points'],
            batch_inputs_dict.get('elastic_coords', None))    #shape:[75609,4] [75609,3] [630057] value: [128,128,151]
        x = spconv.SparseConvTensor(
            features, coordinates, spatial_shape, len(batch_data_samples))  #SparseConvTensor[shape=torch.Size([75609, 3])]

        x = self.extract_feat(x)  # [38584,32] [37025,32]  -> 75609, 32

        #object_class_logits = [self.Classifier(y.mean(dim=0, keepdim=True)) for y in x]
        object_class_logits = []
        for y in x:  
            feat = segment_csr(
                src=y,  
                indptr=torch.arange(0, y.shape[0] + 1, device=y.device),  
                reduce="mean",
            )
            feat = feat.mean(dim=0, keepdim=True)  # [1, num_channels]
            object_class_logits.append(self.Classifier(feat))
        
        # Initialize cumulative losses
        total_semantic_loss_multi = 0  # Cumulative multi-class semantic loss
        batch_size = len(batch_data_samples)

        for i in range(batch_size):

            species_label = batch_data_samples[i].gt_pts_seg.species_label  # Semantic labels (per-point)
            # Calculate multi-class semantic loss using nll_loss
            loss_weight_tensor = torch.tensor(self.loss_weight_species, dtype=torch.float32, device=object_class_logits[i].device)
            self.semantic_loss_multi = torch.nn.functional.nll_loss(
                object_class_logits[i], species_label[0].view(-1).to(torch.int64),weight=loss_weight_tensor
            )
            total_semantic_loss_multi += self.semantic_loss_multi

        # Average the accumulated losses over the batch
        total_semantic_loss_multi /= batch_size

        # Add to total loss
        loss_final = {
            'semantic_loss_multi': total_semantic_loss_multi  
        }
        return loss_final
    ...
    def loss(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        for y in x:  
            feat = y.mean(dim=0, keepdim=True)  # shape: (1, C)
            object_class_features.append(feat)

        object_class_features = torch.cat(object_class_features, dim=0)  # shape: (B, C)

        object_class_logits = self.Classifier(object_class_features)  # shape: (B, num_classes)

        species_labels = torch.stack([sample.gt_pts_seg.species_label[0] for sample in batch_data_samples]).to(torch.int64)  # shape: (B,)

        loss_weight_tensor = torch.tensor(self.loss_weight_species, dtype=torch.float32, device=object_class_logits.device)
        total_semantic_loss_multi = torch.nn.functional.nll_loss(object_class_logits, species_labels, weight=loss_weight_tensor)
    ...
    def predict(self, batch_inputs_dict, batch_data_samples, **kwargs):
        lidar_path = batch_data_samples[0].pts_path
        base_name = os.path.basename(lidar_path)
        current_filename = os.path.splitext(base_name)[0]
        is_test = True #current_filename.isdigit()
        if is_test:
            coordinates, features, inverse_mapping, spatial_shape = self.collate(
                batch_inputs_dict['points'])
            x = spconv.SparseConvTensor(
            ...
            predicted_labels = [logit.argmax(dim=-1).item() for logit in object_class_logits]

            os.makedirs('/workspace/work_dirs/load_FF3D_forspecies_speedup_continue1824_continue3690/test_full_inventory_paper', exist_ok=True)
            output_file = os.path.join('/workspace/work_dirs/load_FF3D_forspecies_speedup_continue1824_continue3690/test_full_inventory_paper', 'prediction_test_full_inventory_paper_2700_2.csv')

            write_header = not os.path.exists(output_file) or os.path.getsize(output_file) == 0


            allowed_names = ['Picea_abies', 'Pinus_sylvestris', 'Betula_pendula']
            allowed_idx = [self.class_names.index(n) for n in allowed_names]

            save_dir = '/workspace/work_dirs/load_FF3D_forspecies_speedup_continue1824_continue3690/test_full_inventory_paper'
            os.makedirs(save_dir, exist_ok=True)
            output_file = os.path.join(save_dir, 'prediction_test_full_inventory_paper_2790_2.csv')

            write_header = not os.path.exists(output_file) or os.path.getsize(output_file) == 0

            with open(output_file, 'a') as f:
                if write_header:
                    f.write("treeID,predicted_species,probability,"
                            "predicted_species_3classes,probability_3classes\n")

                for i, (logit, data_sample) in enumerate(zip(object_class_logits, batch_data_samples)):
                    logits_all = logit.squeeze()

                    # 
                    probs_all = torch.softmax(logits_all, dim=0)           # shape [33]
                    pred_global_idx = int(torch.argmax(probs_all).item())  # 
                    pred_name_all = self.class_names[pred_global_idx]
                    pred_prob_all = float(probs_all[pred_global_idx].item())

                    #
                    logits_sub = logits_all[allowed_idx]                   # 3class
                    probs_sub = torch.softmax(logits_sub, dim=0)           # shape [3]
                    sub_best = int(torch.argmax(probs_sub).item())
                    pred_idx_sub = allowed_idx[sub_best]                   # 
                    pred_name_sub = self.class_names[pred_idx_sub]
                    pred_prob_sub = float(probs_sub[sub_best].item())

                    # -------- treeID --------
                    tree_id = os.path.splitext(os.path.basename(batch_data_samples[0].pts_path))[0]

                    f.write(f"{tree_id},{pred_name_all},{pred_prob_all:.6f},"
                            f"{pred_name_sub},{pred_prob_sub:.6f}\n")

                    # save data_sample
                    data_sample.pred_species_label = pred_global_idx
            
            for i, data_sample in enumerate(batch_data_samples):
                data_sample.pred_species_label = predicted_labels[i]
            return batch_data_samples
        else:
            coordinates, features, inverse_mapping, spatial_shape = self.collate(
                batch_inputs_dict['points'])
    ...
    def vote_semantic_labels(all_pre_sem, originids, cylinder_current_semantic_pre):
        if hasattr(originids, "ndim"):
            if originids.ndim == 0 or (originids.ndim == 1 and len(originids) == 1):  
                originids = [originids.item()]
        elif not hasattr(originids, "__iter__"):  
            originids = [originids]

        # cylinder_current_semantic_pre 
        if hasattr(cylinder_current_semantic_pre, "ndim"):
            if cylinder_current_semantic_pre.ndim == 0 or (cylinder_current_semantic_pre.ndim == 1 and len(cylinder_current_semantic_pre) == 1):  
                cylinder_current_semantic_pre = [cylinder_current_semantic_pre.item()]
        elif not hasattr(cylinder_current_semantic_pre, "__iter__"):  
...
@MODELS.register_module()
class ForestFormerDownstream_Age(Base3DDetector):
    r"""ForAge dataset.
    ...
    def __init__(self,
        ...
        # Ensure all age_weights keys are int
        self.age_weights = {int(k): v for k, v in self.age_weights.items()}

        # Age Regression Head
        self.AgeRegressor = torch.nn.Sequential(
            torch.nn.Linear(num_channels, 256),
            torch.nn.LayerNorm(256),
            torch.nn.ReLU(inplace=True),
            torch.nn.Dropout(p=0.5),
    ...
    def loss22(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        for y in x:
            ...
            age_preds.append(self.AgeRegressor(feat).squeeze(-1))  # [batch_size]
        
        total_age_loss = 0
        batch_size = len(batch_data_samples)
        #self.regression_loss = torch.nn.SmoothL1Loss(beta=self.fixed_beta)  # Fixed beta value
        #self.regression_loss = torch.nn.L1Loss() 
        self.regression_loss = torch.nn.MSELoss()

        for i in range(batch_size):
            age_label = batch_data_samples[i].gt_pts_seg.age_label.float().to(age_preds[i].device)
            #age_pred = age_preds[i]
            age_pred = torch.relu(age_preds[i]) 

            # Apply log1p transformation for normalization
            #age_label_log = torch.log1p(age_label)
            #age_pred_log = torch.log1p(age_pred)

            # Compute Smooth L1 Loss
            loss = self.regression_loss(age_pred, age_label)
            #loss = mse_loss(age_pred, age_label)

            # Convert age_label to int
            age_int = int(round(age_label.item()))

            # Compute Weighted Loss (using fixed age_weights dictionary)
            weight = self.age_weights.get(age_int, 1.0)  # Default weight is 1.0 if age is not found
            
            weighted_loss = loss * (weight + 0.5) #* 10 
            #weighted_loss = loss * weight * 100
            total_age_loss += weighted_loss
            #total_age_loss += loss

        total_age_loss /= batch_size

        loss_final = {
            'age_regression_loss': total_age_loss
        }
        return loss_final
    ...
    def loss(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        x = self.extract_feat(x)  # list of tensors

        # vectorized mean feature extraction
        object_features = []
        for y in x:
            feat = y.mean(dim=0, keepdim=True)  # (1, C)
            object_features.append(feat)

        object_features = torch.cat(object_features, dim=0)  # (B, C)

        # batch predict age
        age_preds = self.AgeRegressor(object_features).squeeze(-1)  # (B,)
        age_preds = torch.relu(age_preds)

        # collect ground truth labels
        age_labels = torch.stack([sample.gt_pts_seg.age_label.float().to(age_preds.device)
                                for sample in batch_data_samples]).squeeze(-1)

        # compute MSE loss batch-wise
        self.regression_loss = torch.nn.MSELoss()
        base_loss = self.regression_loss(age_preds, age_labels)  # already mean by default

        # compute weights per sample
        age_int_labels = torch.round(age_labels).long().cpu().numpy()
        weights = torch.tensor(
            [self.age_weights.get(age_int.item(), 1.0) for age_int in age_int_labels],
            dtype=torch.float32, device=age_preds.device
        )
        ...
        total_age_loss = weighted_loss.mean()

        return {'age_regression_loss': total_age_loss}

    def predict(self, batch_inputs_dict, batch_data_samples, **kwargs):
        lidar_path = batch_data_samples[0].pts_path
        base_name = os.path.basename(lidar_path)
        current_filename = os.path.splitext(base_name)[0]
        is_test = current_filename.isdigit() #True 
        if is_test:
            coordinates, features, inverse_mapping, spatial_shape = self.collate(
                batch_inputs_dict['points'])
            x = spconv.SparseConvTensor(
            ...
            age_predictions = [self.AgeRegressor(y.mean(dim=0, keepdim=True)) for y in x]

            os.makedirs(self.test_output_dir, exist_ok=True)
            if self.test_or_val == 'test':
                output_file = os.path.join(self.test_output_dir, 'test.csv')
            elif self.test_or_val == 'val':
                output_file = os.path.join(self.test_output_dir, 'val.csv')

            write_header = not os.path.exists(output_file) or os.path.getsize(output_file) == 0

            with open(output_file, 'a') as f:
                if write_header:
                    f.write("treeID,predicted_age,age_label\n") 
                for i, data_sample in enumerate(batch_data_samples):
                    data_sample.pred_age_label = age_predictions[i]

                    tree_id = os.path.basename(batch_data_samples[0].pts_path)
                    tree_id = os.path.splitext(tree_id)[0] 
                    predicted_age = age_predictions[i].squeeze().cpu().item()
                    age_label = batch_data_samples[i].gt_pts_seg.age_label.cpu().item()

                    f.write(f"{tree_id},{predicted_age:.2f},{age_label}\n")
...
@MODELS.register_module()
class ForestFormerDownstream_dbh(Base3DDetector):
    r"""Fordbh dataset.
    ...
    def __init__(self,
        ...
        self._init_layers(in_channels, num_channels)
        self.fixed_beta = fixed_beta
        self.dbh_weights = dbh_weights
        self.test_output_dir = test_output_dir
        self.test_or_val = test_or_val
        # Ensure all dbh_weights keys are int
        #self.dbh_weights = {int(k): v for k, v in self.dbh_weights.items()}

        # dbh Regression Head
        self.dbhRegressor = torch.nn.Sequential(
            torch.nn.Linear(num_channels, 256),
            torch.nn.LayerNorm(256),
            torch.nn.ReLU(inplace=True),
            torch.nn.Dropout(p=0.5),
    ...
    def get_dbh_weight(self, dbh_value, dbh_weights_dict):
        for interval, weight in dbh_weights_dict.items():
            low, high = map(float, interval.split('-'))
            if low <= dbh_value < high:
                return weight
        return 1.0
    ...
    def loss22(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        for y in x:
            ...
            feat = feat.mean(dim=0, keepdim=True)
            dbh_preds.append(self.dbhRegressor(feat).squeeze(-1))  # [batch_size]
        ...
        for i in range(batch_size):
            dbh_label = batch_data_samples[i].gt_pts_seg.dbh_label.float().to(dbh_preds[i].device)
            #dbh_pred = dbh_preds[i]
            #dbh_pred = torch.relu(dbh_preds[i]) 

            dbh_label_cm = dbh_label * 100  #m to cm
            dbh_pred_cm = torch.relu(dbh_preds[i]) #* 100

            # Apply log1p transformation for normalization
            #dbh_label_log = torch.log1p(dbh_label)
            #dbh_pred_log = torch.log1p(dbh_pred)

            # Compute Smooth L1 Loss
            loss = self.regression_loss(dbh_pred_cm, dbh_label_cm)
            #loss = mse_loss(dbh_pred, dbh_label)

            # Convert dbh_label to int
            dbh_float = dbh_label.item()

            # Compute Weighted Loss (using fixed dbh_weights dictionary)
            weight = self.get_dbh_weight(dbh_float, self.dbh_weights)
        
            weighted_loss = loss * (weight + 0.5)
            #weighted_loss = loss * weight * 100
            total_dbh_loss += weighted_loss
            #total_dbh_loss += loss

        total_dbh_loss /= batch_size

        loss_final = {
            'dbh_regression_loss': total_dbh_loss
        }
        return loss_final
    ...
    def loss(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        object_features = []
        for y in x:
            feat = y.mean(dim=0, keepdim=True)
            object_features.append(feat)

        object_features = torch.cat(object_features, dim=0)  # (B, C)

        # 
        dbh_preds = self.dbhRegressor(object_features).squeeze(-1)  # (B,)
        dbh_preds_cm = torch.relu(dbh_preds)  #

        #gt dbh label
        dbh_labels = torch.stack([sample.gt_pts_seg.dbh_label.float().to(dbh_preds_cm.device)
                                for sample in batch_data_samples]).squeeze(-1)
        dbh_labels_cm = dbh_labels * 100  # cm 

        # MSE loss
        self.regression_loss = torch.nn.MSELoss()
        base_loss = self.regression_loss(dbh_preds_cm, dbh_labels_cm)

        # 
        dbh_floats = dbh_labels.cpu().numpy()  # (B,)
        weights_list = [self.get_dbh_weight(dbh.item(), self.dbh_weights) for dbh in dbh_floats]
        weights = torch.tensor(weights_list, dtype=torch.float32, device=dbh_preds_cm.device)

        weighted_loss = base_loss * weights

        total_dbh_loss = weighted_loss.mean()

        return {'dbh_regression_loss': total_dbh_loss}
    ...
    def predict(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        base_name = os.path.basename(lidar_path)
        current_filename = os.path.splitext(base_name)[0]
        is_test = True #current_filename.isdigit() #True
        if is_test:
            coordinates, features, inverse_mapping, spatial_shape = self.collate(
                batch_inputs_dict['points'])
            x = spconv.SparseConvTensor(
            ...
            dbh_predictions = [self.dbhRegressor(y.mean(dim=0, keepdim=True)) for y in x]
            ...
            #os.makedirs('/workspace/work_dirs/load_FF3D_fordbh_noscale', exist_ok=True)
            #output_file = os.path.join('/workspace/work_dirs/load_FF3D_fordbh_noscale', 'predictions_test_best_600.csv')

            write_header = not os.path.exists(output_file) or os.path.getsize(output_file) == 0

            with open(output_file, 'a') as f:
                if write_header:
                    f.write("treeID,predicted_dbh,dbh_label\n") 
                for i, data_sample in enumerate(batch_data_samples):
                    data_sample.pred_dbh_label = dbh_predictions[i]

                    tree_id = os.path.basename(batch_data_samples[0].pts_path)
                    tree_id = os.path.splitext(tree_id)[0] 
                    predicted_dbh = dbh_predictions[i].squeeze().cpu().item()
                    predicted_dbh = predicted_dbh/100
                    dbh_label = batch_data_samples[i].gt_pts_seg.dbh_label.cpu().item()

                    f.write(f"{tree_id},{predicted_dbh:.2f},{dbh_label}\n")
            
            for i, data_sample in enumerate(batch_data_samples):
                data_sample.pred_dbh_label = dbh_predictions[i]/100
            return batch_data_samples
        else:
            coordinates, features, inverse_mapping, spatial_shape = self.collate(
                batch_inputs_dict['points'])
...
@MODELS.register_module()
class ForestFormerDownstream_height(Base3DDetector):
    r"""Forheight dataset.
    ...
    def __init__(self,
        ...
        self.test_output_dir = test_output_dir
        self.test_or_val = test_or_val
        # Ensure all height_weights keys are int
        #self.height_weights = {int(k): v for k, v in self.height_weights.items()}

        # height Regression Head
        self.heightRegressor = torch.nn.Sequential(
            torch.nn.Linear(num_channels, 256),
            torch.nn.LayerNorm(256),
            torch.nn.ReLU(inplace=True),
            torch.nn.Dropout(p=0.5),
    ...
    def get_height_weight(self, height_value, height_weights_dict):
        for interval, weight in height_weights_dict.items():
            low, high = map(float, interval.split('-'))
            if low <= height_value < high:
                return weight
        return 1.0
    ...
    def loss22(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        for y in x:
            ...
            feat = feat.mean(dim=0, keepdim=True)
            height_preds.append(self.heightRegressor(feat).squeeze(-1))  # [batch_size]
        
        height_preds = torch.stack(height_preds)  # [batch_size]
        height_labels = torch.stack([
        sample.gt_pts_seg.height_label.float().to(height_preds.device)
            for sample in batch_data_samples
        ])  # [batch_size]
        
        
        #height_labels_log = torch.log1p(height_labels)
        #height_preds_log = torch.log1p(height_preds)
        self.regression_loss = torch.nn.MSELoss(reduction='none') 
        losses = self.regression_loss(height_preds, height_labels)

        weights = []
        for label in height_labels:
            h_float = label.item()
            weight = self.get_height_weight(h_float, self.height_weights)
            weights.append(weight)
        weights = torch.tensor(weights, device=height_preds.device)  # [batch_size]

        weighted_losses = losses * weights * 100

        total_loss = weighted_losses.mean()

        loss_final = {
            'height_regression_loss': total_loss
        }
        return loss_final
    ...
    def loss(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        # 
        height_preds = self.heightRegressor(object_features).squeeze(-1)  # (B,)
        ...
        # 计算 per-sample MSE
        self.regression_loss = torch.nn.MSELoss(reduction='none')
        losses = self.regression_loss(height_preds, height_labels)  # (B,)

        # 
        height_floats = height_labels.cpu().numpy()
        weights_list = [self.get_height_weight(h.item(), self.height_weights) for h in height_floats]
        weights = torch.tensor(weights_list, dtype=torch.float32, device=height_preds.device)

        weighted_losses = losses * weights

        total_loss = weighted_losses.mean()

        return {'height_regression_loss': total_loss}
    ...
    def predict(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        if is_test:
            ...
            height_predictions = [self.heightRegressor(y.mean(dim=0, keepdim=True)) for y in x]
            ...
            #os.makedirs('/workspace/work_dirs/load_FF3D_forheight', exist_ok=True)
            #output_file = os.path.join('/workspace/work_dirs/load_FF3D_forheight', 'predictions_test_last_994.csv')

            write_header = not os.path.exists(output_file) or os.path.getsize(output_file) == 0

            with open(output_file, 'a') as f:
                if write_header:
                    f.write("treeID,predicted_height,height_label\n") 
                for i, data_sample in enumerate(batch_data_samples):
                    data_sample.pred_height_label = height_predictions[i]

                    tree_id = os.path.basename(batch_data_samples[0].pts_path)
                    tree_id = os.path.splitext(tree_id)[0] 
                    predicted_height = height_predictions[i].squeeze().cpu().item()
                    predicted_height = predicted_height
                    height_label = batch_data_samples[i].gt_pts_seg.height_label.cpu().item()

                    f.write(f"{tree_id},{predicted_height:.2f},{height_label}\n")
...
@MODELS.register_module()
class ForestFormerDownstream_4heads(Base3DDetector):
    r"""ForSpecies dataset.
    ...
    def __init__(self,
        ...
        self.loss_weight_dbh = loss_weight_dbh
        self.loss_weight_height = loss_weight_height
        self.regression_loss = torch.nn.MSELoss()
        self.test_output_dir = test_output_dir
        self.test_or_val = test_or_val

        self.age_weights = {int(k): v for k, v in self.loss_weight_age.items()}
...
'''
        ...
        species_loss_sum = torch.tensor(0.0, device=device)
        age_loss_sum = torch.tensor(0.0, device=device)
        dbh_loss_sum = torch.tensor(0.0, device=device)
        height_loss_sum = torch.tensor(0.0, device=device)
        ...
        for i, y in enumerate(x):  
            ...
            #sample_loss = 0.0
            #count = 0
            if hasattr(sample.gt_pts_seg, 'species_label'):
                pred = self.Classifier(feat)
                species_label = sample.gt_pts_seg.species_label
                loss_weight_tensor = torch.tensor(self.loss_weight_species, dtype=torch.float32, device=pred.device)
                loss_species = torch.nn.functional.nll_loss(
                    pred, species_label[0].view(-1).to(torch.int64), weight=loss_weight_tensor
                )
                species_loss_sum += loss_species
                count_species += 1

            if hasattr(sample.gt_pts_seg, 'age_label'):
                pred = self.AgeRegressor(feat)
                age_pred = torch.relu(pred) 
                age_label = sample.gt_pts_seg.age_label.float().to(pred.device)
                loss_age  = self.regression_loss(age_pred[0], age_label)
                age_int = int(round(age_label.item()))
                weight = self.age_weights.get(age_int, 1.0)  # Default weight is 1.0 if age is not found
                weighted_loss_age = loss_age * (weight + 0.5)
                age_loss_sum += weighted_loss_age
                count_age += 1

            if hasattr(sample.gt_pts_seg, 'dbh_label'):
                pred = self.dbhRegressor(feat)
                dbh_pred_cm = torch.relu(pred)
                dbh_label = sample.gt_pts_seg.dbh_label.float().to(pred.device)
                dbh_label_cm = dbh_label * 100
                loss_dbh = self.regression_loss(dbh_pred_cm[0], dbh_label_cm)
                dbh_float = dbh_label.item()
                weight = self.get_dbh_weight(dbh_float, self.loss_weight_dbh)
                weighted_loss_dbh = loss_dbh * (weight + 0.5)
                dbh_loss_sum += weighted_loss_dbh
                count_dbh += 1

            if hasattr(sample.gt_pts_seg, 'height_label'):
                height_pred = self.heightRegressor(feat)
                #height_pred = torch.relu(pred)
                height_label = sample.gt_pts_seg.height_label.float().to(height_pred.device)
                loss_height = self.regression_loss(height_pred[0], height_label)
                height_float = height_label.item()
                weight = self.get_dbh_weight(height_float, self.loss_weight_height)
                weighted_loss_height = loss_height * (weight + 0.5)
                height_loss_sum += weighted_loss_height
                count_height += 1

            #if count > 0:
            #   total_loss += sample_loss / count  # average per-sample loss

        species_loss = species_loss_sum / count_species if count_species > 0 else torch.tensor(0.0, device=device)
        age_loss = age_loss_sum / count_age if count_age > 0 else torch.tensor(0.0, device=device)
        dbh_loss = dbh_loss_sum / count_dbh if count_dbh > 0 else torch.tensor(0.0, device=device)
        height_loss = height_loss_sum / count_height if count_height > 0 else torch.tensor(0.0, device=device)
    ...
    def loss(self, batch_inputs_dict, batch_data_samples, **kwargs):
        """Batch-wise accelerated multi-task loss function."""
        ...
        for y in x:
            feat = y.mean(dim=0, keepdim=True)
            object_features.append(feat)
        object_features = torch.cat(object_features, dim=0)  # shape (B, C)

        
        species_preds = self.Classifier(object_features)  # (B, num_species)
        age_preds = torch.relu(self.AgeRegressor(object_features).squeeze(-1))  # (B,)
        dbh_preds = torch.relu(self.dbhRegressor(object_features).squeeze(-1))  # (B,)
        height_preds = self.heightRegressor(object_features).squeeze(-1)  # (B,)

        
        species_labels, species_mask = [], []
        age_labels, age_mask = [], []
        dbh_labels, dbh_mask = [], []
        height_labels, height_mask = [], []

        for sample in batch_data_samples:
            # species label
            if hasattr(sample.gt_pts_seg, 'species_label'):
                label = sample.gt_pts_seg.species_label[0].to(torch.int64).item()
                species_labels.append(label)
                species_mask.append(True)
            else:
                species_labels.append(-1)
                species_mask.append(False)

            # age label
            if hasattr(sample.gt_pts_seg, 'age_label'):
                label = sample.gt_pts_seg.age_label.float().item()
                age_labels.append(label)
                age_mask.append(True)
            else:
                age_labels.append(0.0)
                age_mask.append(False)

            # dbh label
            if hasattr(sample.gt_pts_seg, 'dbh_label'):
                label = sample.gt_pts_seg.dbh_label.float().item() * 100  # m -> cm
                dbh_labels.append(label)
                dbh_mask.append(True)
            else:
                dbh_labels.append(0.0)
                dbh_mask.append(False)

            # height label
            if hasattr(sample.gt_pts_seg, 'height_label'):
                label = sample.gt_pts_seg.height_label.float().item()
                height_labels.append(label)
                height_mask.append(True)
            else:
                height_labels.append(0.0)
                height_mask.append(False)

        # 
        species_labels = torch.tensor(species_labels, dtype=torch.int64, device=device)
        species_mask = torch.tensor(species_mask, dtype=torch.bool, device=device)
        age_labels = torch.tensor(age_labels, dtype=torch.float32, device=device)
        age_mask = torch.tensor(age_mask, dtype=torch.bool, device=device)
        dbh_labels = torch.tensor(dbh_labels, dtype=torch.float32, device=device)
        dbh_mask = torch.tensor(dbh_mask, dtype=torch.bool, device=device)
        height_labels = torch.tensor(height_labels, dtype=torch.float32, device=device)
        ...
        if species_mask.sum() > 0:
            valid_species_preds = species_preds[species_mask]
            valid_species_labels = species_labels[species_mask]
            loss_weight_tensor = torch.tensor(self.loss_weight_species, dtype=torch.float32, device=device)
            species_loss = torch.nn.functional.nll_loss(valid_species_preds, valid_species_labels, weight=loss_weight_tensor)
            loss_dict['species_loss'] = species_loss

        # age loss
        if age_mask.sum() > 0:
            valid_age_preds = age_preds[age_mask]
            valid_age_labels = age_labels[age_mask]
            base_age_loss = torch.nn.functional.mse_loss(valid_age_preds, valid_age_labels, reduction='none')
            weights = torch.tensor(
                [self.age_weights.get(int(round(lbl.item())), 1.0) for lbl in valid_age_labels],
                dtype=torch.float32, device=device)
            age_loss = (base_age_loss * weights).mean()
            loss_dict['age_loss'] = age_loss

        # dbh loss
        if dbh_mask.sum() > 0:
            valid_dbh_preds = dbh_preds[dbh_mask]
            valid_dbh_labels = dbh_labels[dbh_mask]
            base_dbh_loss = torch.nn.functional.mse_loss(valid_dbh_preds, valid_dbh_labels, reduction='none')
            weights = torch.tensor(
                [self.get_dbh_weight(lbl.item() / 100, self.loss_weight_dbh) for lbl in valid_dbh_labels],
                dtype=torch.float32, device=device)
            dbh_loss = (base_dbh_loss * weights).mean()
            loss_dict['dbh_loss'] = dbh_loss

        # height loss
        if height_mask.sum() > 0:
            valid_height_preds = height_preds[height_mask]
            valid_height_labels = height_labels[height_mask]
            base_height_loss = torch.nn.functional.mse_loss(valid_height_preds, valid_height_labels, reduction='none')
            weights = torch.tensor(
                [self.get_dbh_weight(lbl.item(), self.loss_weight_height)  for lbl in valid_height_labels],
                dtype=torch.float32, device=device)
            height_loss = (base_height_loss * weights).mean()
            loss_dict['height_loss'] = height_loss

        return loss_dict

    def _prepare_csv(self, path, header_cols):
    ...
    def predict(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        for y in x:
            ...
            pred_species = self.Classifier(feat)
            pred_species_label = pred_species.argmax(dim=-1).item()

            pred_dbh = self.dbhRegressor(feat).squeeze().item() / 100.0  # Convert cm -> m
            pred_height = self.heightRegressor(feat).squeeze().item()
            pred_age = self.AgeRegressor(feat).squeeze().item()

            predictions.append({
                'species_label': pred_species_label,
                'dbh_pred': pred_dbh,
                'height_pred': pred_height,
                'age_pred': pred_age,
            })
        ...
        if is_test:
            ...
            has_height_label  = hasattr(batch_data_samples[0].gt_pts_seg, 'height_label')
            has_dbh_label     = hasattr(batch_data_samples[0].gt_pts_seg, 'dbh_label')
            has_age_label     = hasattr(batch_data_samples[0].gt_pts_seg, 'age_label')
            has_species_label = hasattr(batch_data_samples[0].gt_pts_seg, 'species_label')

            species_path = os.path.join(base_dir, f"{split}_species.csv")
            height_path  = os.path.join(base_dir, f"{split}_height.csv")
            dbh_path     = os.path.join(base_dir, f"{split}_dbh.csv")
            age_path     = os.path.join(base_dir, f"{split}_age.csv")

            f_species = self._prepare_csv(species_path, ["treeID", "predicted_species"])
            f_height  = self._prepare_csv(height_path,  ["treeID", "predicted_height", "height_label"])
            f_dbh     = self._prepare_csv(dbh_path,     ["treeID", "predicted_dbh", "dbh_label"])
            f_age     = self._prepare_csv(age_path,     ["treeID", "predicted_age", "age_label"])

            try:
                for i, data_sample in enumerate(batch_data_samples):
                    pred = predictions[i]
                    tree_id = os.path.splitext(os.path.basename(data_sample.pts_path))[0]

                    # ===== species =====
                    species_idx = pred['species_label']
                    if has_species_label:
                        label_name = self.class_names[species_idx]
                        f_species.write(f"{tree_id},{label_name}\n")
                        data_sample.pred_species_label = species_idx

                    # ===== height =====
                    pred_height = float(pred['height_pred'])
                    if has_height_label:
                        height_label = float(data_sample.gt_pts_seg.height_label.cpu().item())
                        f_height.write(f"{tree_id},{pred_height:.2f},{height_label}\n")
                        data_sample.pred_height = pred_height

                    # ===== dbh =====
                    pred_dbh = float(pred['dbh_pred'])
                    if has_dbh_label:
                        dbh_label = float(data_sample.gt_pts_seg.dbh_label.cpu().item())
                        f_dbh.write(f"{tree_id},{pred_dbh:.2f},{dbh_label}\n")
                        data_sample.pred_dbh = pred_dbh

                    # ===== age =====
                    pred_age = float(pred['age_pred'])
                    if has_age_label:
                        age_label = float(data_sample.gt_pts_seg.age_label.cpu().item())
                        f_age.write(f"{tree_id},{pred_age:.2f},{age_label}\n")
                        data_sample.pred_age = pred_age
            finally:
                f_species.close()
                f_height.close()
                f_dbh.close()
            ...
            for i, data_sample in enumerate(batch_data_samples):
                pred = predictions[i]
                data_sample.pred_species_label = pred['species_label']
                data_sample.pred_dbh = pred['dbh_pred']
                data_sample.pred_height = pred['height_pred']
                data_sample.pred_age = pred['age_pred']
