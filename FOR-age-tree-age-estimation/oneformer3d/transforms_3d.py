...
class GridSample(BaseTransform):
    ...
    def transform(self, input_dict):
        ...
        #np.savetxt("points_xyz_gridsample.txt", input_dict["points"].cpu().numpy(), fmt="%.6f", delimiter=" ")
...
@TRANSFORMS.register_module()
class LoadPointsAndSpeciesLabelFromFile(BaseTransform):
    def __init__(self,
                 coord_type: str,
                 load_dim: int = 6,
                 use_dim: Union[int, List[int]] = [0, 1, 2],
                 shift_height: bool = False,
                 shift_height_xyz: bool = False, 
                 use_color: bool = False,
                 norm_intensity: bool = False,
                 backend_args: Optional[dict] = None) -> None:
        self.shift_height = shift_height
        self.shift_height_xyz = shift_height_xyz
    ...
    def transform(self, results: dict) -> dict:
        ...
        """
        pts_file_path = results['pts_path']
        points = self._load_points(pts_file_path)
        points = points.reshape(-1, self.load_dim)
        points = points[:, self.use_dim]

        if self.shift_height_xyz:
            x_min, y_min, z_min = np.min(points[:, :3], axis=0)
            x_max, y_max, _ = np.max(points[:, :3], axis=0)
            
            bbox_center_x = (x_max + x_min) / 2
            bbox_center_y = (y_max + y_min) / 2

            points[:, 0] -= bbox_center_x 
            points[:, 1] -= bbox_center_y  
            points[:, 2] -= z_min  

        #np.savetxt("points_xyz.txt", points, fmt="%.6f", delimiter=" ")
        ...
        species_label_path = results['label_path']
        species_label = np.fromfile(species_label_path, dtype=np.int64)
        results['species_label'] = species_label
    ...
    def __repr__(self) -> str:
        ...
        repr_str += f'shift_height_xyz={self.shift_height_xyz}, '
...
@TRANSFORMS.register_module()
class LoadPointsAndAgeLabelFromFile(BaseTransform):
    def __init__(self,
                 coord_type: str,
                 load_dim: int = 6,
                 use_dim: Union[int, List[int]] = [0, 1, 2],
    ...
    def transform(self, results: dict) -> dict:
        ...
        #np.savetxt("points_xyz_age.txt", points, fmt="%.6f", delimiter=" ")
        if self.norm_intensity:
            assert len(self.use_dim) >= 4, \
                f'When using intensity norm, expect used dimensions >= 4, got {len(self.use_dim)}'  # noqa: E501
            points[:, 3] = np.tanh(points[:, 3])
        ...
        age_label_path = results['label_path']
        age_label = np.fromfile(age_label_path, dtype=np.int64)
        results['age_label'] = age_label
...
@TRANSFORMS.register_module()
class LoadPointsAnddbhLabelFromFile(BaseTransform):
    def __init__(self,
                 coord_type: str,
                 load_dim: int = 6,
                 use_dim: Union[int, List[int]] = [0, 1, 2],
    ...
    def transform(self, results: dict) -> dict:
        ...
        #np.savetxt("points_xyz_dbh.txt", points, fmt="%.6f", delimiter=" ")
        if self.norm_intensity:
            assert len(self.use_dim) >= 4, \
                f'When using intensity norm, expect used dimensions >= 4, got {len(self.use_dim)}'  # noqa: E501
            points[:, 3] = np.tanh(points[:, 3])
        ...
        dbh_label_path = results['label_path']
        dbh_label = np.fromfile(dbh_label_path, dtype=np.float32)
        results['dbh_label'] = dbh_label
...
@TRANSFORMS.register_module()
class LoadPointsAndheightLabelFromFile(BaseTransform):
    def __init__(self,
                 coord_type: str,
                 load_dim: int = 6,
                 use_dim: Union[int, List[int]] = [0, 1, 2],
    ...
    def transform(self, results: dict) -> dict:
        ...
        #np.savetxt("points_xyz_height.txt", points, fmt="%.6f", delimiter=" ")
        if self.norm_intensity:
            assert len(self.use_dim) >= 4, \
                f'When using intensity norm, expect used dimensions >= 4, got {len(self.use_dim)}'  # noqa: E501
            points[:, 3] = np.tanh(points[:, 3])
        ...
        height_label_path = results['label_path']
        height_label = np.fromfile(height_label_path, dtype=np.float32)
        results['height_label'] = height_label
...
@TRANSFORMS.register_module()
class RandomGridSample(BaseTransform):
    def __init__(self, grid_size_range=(0.02, 0.2), mode="train", hash_type="fnv"):
    ...
    def transform(self, input_dict):
        ...
        grid_size = random.uniform(self.grid_size_range[0], self.grid_size_range[1])
        scaled_points = points.tensor / grid_size
        grid_points = torch.floor(scaled_points).int()
        min_points = torch.min(grid_points, dim=0).values
        grid_points -= min_points
        scaled_points -= min_points
        min_points = min_points * grid_size
