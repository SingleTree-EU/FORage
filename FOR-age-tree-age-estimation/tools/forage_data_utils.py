...
class forageData(object):
    """forage_instance_data data.
    Generate forage_instance_data infos for forage_instance_data.
    ...
    def get_infos(self, num_workers=4, has_label=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            pts_filename = osp.join(self.root_dir, 'forage_instance_data',
            ...
            pts_age_label_path = osp.join(
                self.root_dir, 'forage_instance_data',
                f'{sample_idx}_age_label.npy')

            pts_age_label = np.load(pts_age_label_path).astype(
                np.int64)

            mmengine.mkdir_or_exist(
                osp.join(self.save_path, 'age_label'))

            pts_age_label.tofile(
                osp.join(self.save_path, 'age_label',
                            f'{sample_idx}.bin'))

            info['age_label_path'] = osp.join(
                'age_label', f'{sample_idx}.bin')
