...
def update_forspecies20k_infos(pkl_path, out_dir):
    print(f'{pkl_path} will be modified.')
    if out_dir in pkl_path:
        print(f'Warning, you may overwriting '
              f'the original data {pkl_path}.')
    ...
    for ori_info_dict in mmengine.track_iter_progress(data_list):
        ...
        temp_data_info['lidar_points']['lidar_path'] = Path(
            ori_info_dict['pts_path']).name
        if 'species_label_path' in ori_info_dict:
            temp_data_info['species_label_path'] = Path(
                ori_info_dict['species_label_path']).name
        #if 'super_pts_path' in ori_info_dict:
        #    temp_data_info['super_pts_path'] = Path(
        #        ori_info_dict['super_pts_path']).name
    ...
    metainfo = dict()
    metainfo['categories'] = {k: i for i, k in enumerate(METAINFO['classes'])}
    metainfo['dataset'] = 'forspecies20k'
    metainfo['info_version'] = '1.1'
...
def update_forage_infos(pkl_path, out_dir):
    print(f'{pkl_path} will be modified.')
    if out_dir in pkl_path:
        print(f'Warning, you may overwriting '
              f'the original data {pkl_path}.')
    ...
    for ori_info_dict in mmengine.track_iter_progress(data_list):
        ...
        temp_data_info['lidar_points']['lidar_path'] = Path(
            ori_info_dict['pts_path']).name
        if 'age_label_path' in ori_info_dict:
            temp_data_info['age_label_path'] = Path(
                ori_info_dict['age_label_path']).name
        #if 'super_pts_path' in ori_info_dict:
        #    temp_data_info['super_pts_path'] = Path(
        #        ori_info_dict['super_pts_path']).name
...
def update_fordbh_infos(pkl_path, out_dir):
    print(f'{pkl_path} will be modified.')
    if out_dir in pkl_path:
        print(f'Warning, you may overwriting '
              f'the original data {pkl_path}.')
    ...
    for ori_info_dict in mmengine.track_iter_progress(data_list):
        ...
        temp_data_info['lidar_points']['lidar_path'] = Path(
            ori_info_dict['pts_path']).name
        if 'dbh_label_path' in ori_info_dict:
            temp_data_info['dbh_label_path'] = Path(
                ori_info_dict['dbh_label_path']).name
        #if 'super_pts_path' in ori_info_dict:
        #    temp_data_info['super_pts_path'] = Path(
        #        ori_info_dict['super_pts_path']).name
...
def update_forheight_infos(pkl_path, out_dir):
    print(f'{pkl_path} will be modified.')
    if out_dir in pkl_path:
        print(f'Warning, you may overwriting '
              f'the original data {pkl_path}.')
    ...
    for ori_info_dict in mmengine.track_iter_progress(data_list):
        ...
        temp_data_info['lidar_points']['lidar_path'] = Path(
            ori_info_dict['pts_path']).name
        if 'height_label_path' in ori_info_dict:
            temp_data_info['height_label_path'] = Path(
                ori_info_dict['height_label_path']).name
        #if 'super_pts_path' in ori_info_dict:
        #    temp_data_info['super_pts_path'] = Path(
        #        ori_info_dict['super_pts_path']).name
...
def update_pkl_infos(dataset, out_dir, pkl_path):
    ...
    elif dataset.lower() == 'forspecies20k':
        update_forspecies20k_infos(pkl_path=pkl_path, out_dir=out_dir)
    elif dataset.lower() == 'forage':
        update_forage_infos(pkl_path=pkl_path, out_dir=out_dir)
    elif dataset.lower() == 'fordbh':
        update_fordbh_infos(pkl_path=pkl_path, out_dir=out_dir)
    elif dataset.lower() == 'forheight':
        update_forheight_infos(pkl_path=pkl_path, out_dir=out_dir)
    else:
